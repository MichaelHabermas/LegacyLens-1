...
*Final PRD. Based on [PRD-V2](research/PRD-V2.md). Aligned with [RAG and Legacy Code Primer](technology/RAG-AND-LEGACY-PRIMER.md), [Tech Stack Guide](technology/TECH-STACK-GUIDE.md), and [Environment and Configuration](technology/ENVIRONMENT-AND-CONFIG.md).*
...
LegacyLens is a RAG-powered system that makes legacy enterprise codebases (COBOL, Fortran) queryable via natural language. This PRD details the implementation roadmap from MVP (24 hours) through final deployment (7 days), following SOLID principles and modular architecture. The epic dependency roadmap below shows the order in which epics build on each other.
...
```mermaid
graph LR
    E0[Epic_0_Foundation] --> E1[Epic_1_Ingestion]
    E1 --> E2[Epic_2_Retrieval]
    E2 --> E3[Epic_3_Generation]
    E3 --> E4[Epic_4_CLI]
    E4 --> E5[Epic_5_Observability]
    E5 --> E6[Epic_6_MVP_Hardening]
    E6 --> PostMVP[Post-MVP]
    PostMVP --> E7[Epic_7_Advanced]
    PostMVP --> E8[Epic_8_Optimization]
    PostMVP --> E9[Epic_9_CI/CD]
    PostMVP --> E10[Epic_10_Demo]
```

### Parallelization

**Parallel-safe** means two epics touch **no shared files or directories**; they can be developed in different branches at the same time without merge conflicts.

- **MVP (after Epic 0)**: Epics **1, 2, 3** can run in parallel. Epic **4** is parallel-safe with Epic 2 and Epic 3. Epic **5** is parallel-safe with Epic 1 and Epic 4. Epic **6** runs after 1–5 (validation; no parallel).
- **Post-MVP**: Epics **7, 8, 9** can run in parallel; Epic **10** is parallel-safe with all (no code changes).

```mermaid
flowchart LR
    subgraph afterE0 [After Epic 0]
        E1[Epic_1]
        E2[Epic_2]
        E3[Epic_3]
        E1 <-.->|parallel-safe| E2
    ...
    subgraph afterE123 [After 1 2 3]
        E4[Epic_4]
        E5[Epic_5]
        E4 <-.->|parallel-safe| E5
    end
...
**Parallelization**: **No parallel** (foundation; all other epics depend on it).

### Epic 0 Overview

Epic 0 establishes the project foundation required for all subsequent development. It defines the repository layout, Cursor AI rules for consistent code generation, environment and configuration management, and the core interfaces and data models that ingestion, retrieval, and generation modules will implement. Completing this epic unblocks Epics 1–6 and ensures SOLID-compliant, type-safe development from the start.

### Dependencies

- None (first epic). Requires only Python 3.11+ and access to the repository.

### Success Criteria

- Repository structure exists with `src/`, `tests/`, `docs/`, `.github/` and `pip install -e .` succeeds.
- `.cursor/rules` exists and documents tech stack, SOLID, git workflow, and Context7 MCP.
- Config module loads and validates all required environment variables; unit tests pass.
- All core interfaces (IParser, IChunker, IEmbedder, IIndexer, ISearcher, IReranker, IContextAssembler, ILLMProvider) and data models (Chunk, Query, SearchResult) are defined and documented.

### Risks and Mitigations

- **Config drift**: Document all env vars in `.env.example` and enforce validation in `Settings` so missing keys fail fast.
- **Interface churn**: Define interfaces in Epic 0 and keep them stable; extend via new implementations rather than changing contracts.

### Foundation Scope

```mermaid
graph TD
    Repo[Repo_Structure] --> Rules[Cursor_Rules]
    Repo --> Config[Env_Config]
    Repo --> Interfaces[Core_Interfaces]
    Rules --> TechStack[Tech_Stack]
    Rules --> SOLID[SOLID_Git_Quality]
    Config --> Settings[Settings_Module]
    Interfaces --> IngestionIf[Ingestion_Interfaces]
    Interfaces --> RetrievalIf[Retrieval_Interfaces]
    Interfaces --> GenerationIf[Generation_Interfaces]
    Interfaces --> Models[Data_Models]
```
...
**Acceptance criteria**: Done when directory structure exists, pyproject.toml and CI workflows are in place, and README documents setup. **Definition of Done**: `pip install -e .` and CI workflows validate.
...
**Parallelization**: **Parallel-safe with**: Epic 2, Epic 3. **Not parallel-safe with**: Epic 4 (shared: `src/cli/`).

### Epic 1 Overview

Epic 1 implements the ingestion pipeline that turns raw legacy code into searchable vector representations. It covers file discovery and preprocessing, COBOL AST parsing for logical boundaries (paragraphs, sections), syntax-aware chunking with overlap, Voyage-code-3 embedding generation with Redis caching, and Pinecone indexing with metadata. This epic is critical for MVP: without it, retrieval and generation have no data to operate on.

### Dependencies

- **Epic 0**: Core interfaces (IParser, IChunker, IEmbedder, IIndexer), data models (Chunk), and config (env vars for Pinecone, Voyage, Redis) must be in place.

### Success Criteria

- GnuCOBOL codebase (10K+ LOC) is fully ingested in under 5 minutes.
- Chunks respect COBOL paragraph/section boundaries with configurable overlap (10–25%).
- Embeddings are generated via Voyage-code-3; cache hits skip API calls.
- All chunks are stored in Pinecone with metadata (file_path, line_start, line_end, scope_chain, language).
- CLI command `legacylens ingest <path>` runs end-to-end with progress and summary stats.

### Risks and Mitigations

- **Voyage API rate limits**: Batch up to 128 chunks per call and implement exponential backoff; use Redis content-hash cache to avoid re-embedding unchanged chunks.
- **Parser failures on malformed code**: Fall back to line-based chunking and log failures so ingestion continues; fix parser incrementally.

### Ingestion Pipeline Flow

```mermaid
graph LR
    Scan[Scan_Files] --> Parse[Parse_COBOL]
    Parse --> Chunk[Chunk_Syntax]
    Chunk --> Embed[Embed_Voyage]
    Embed --> Index[Index_Pinecone]
```
...
**Acceptance criteria**: Done when `legacylens ingest <path>` runs Scan → Parse → Chunk → Embed → Index with progress and completes 10K LOC in under 5 minutes. **Definition of Done**: Orchestrator tests pass with mocked dependencies; performance benchmark meets target.

#### Ingestion Orchestrator Sequence
...
sequenceDiagram
    ...
    Orch->>Scanner: scan_directory
    Scanner-->>Orch: file_paths
    loop For each file
        Orch->>Parser: parse_file
        Parser-->>Orch: parsed_units
        Orch->>Chunker: chunk
        Chunker-->>Orch: chunks
    end
    Orch->>Embedder: embed_batch
    Embedder-->>Orch: vectors
    Orch->>Indexer: upsert
    Indexer-->>Orch: done
```
...
**Parallelization**: **Parallel-safe with**: Epic 1, Epic 3, Epic 4. **Not parallel-safe with**: Epic 5 (shared: retrieval wrapped by tracing).

### Epic 2 Overview

Epic 2 implements the retrieval pipeline that turns a natural language query into a ranked, contextual set of code chunks for the LLM. It covers query processing (parsing, entity extraction, intent detection, embedding), Pinecone hybrid search (vector + BM25) with metadata filtering, Zerank-2 reranking to improve top-k precision, context assembly with file/line citations, and optional agentic multi-query expansion for ambiguous questions. This epic directly supports the >70% top-5 precision and <3s latency targets.

### Dependencies

- **Epic 0**: ISearcher, IReranker, IContextAssembler, Query and SearchResult models; config for Pinecone, Voyage, Redis.
- **Epic 1**: Pinecone index populated with embeddings and metadata so hybrid search and filtering work.

### Success Criteria

- Hybrid search returns top-k (20–50) candidates with vector + keyword scoring and metadata filters.
- Reranker reduces to top-5–10 with normalized scores; end-to-end retrieval latency stays within budget for <3s E2E.
- Assembled context includes file/line citations and optional surrounding lines; format is consistent for generation.
- Optional query expansion produces merged results for ambiguous queries without breaking latency for simple ones.

### Risks and Mitigations

- **Latency creep**: Keep top-k and rerank output sizes configurable; use query embedding cache (Redis, 1h TTL) to avoid redundant Voyage calls.
- **Precision below 70%**: Tune reranker weights and hybrid mix; validate on labeled queries in Epic 6.

### Retrieval Pipeline Flow

```mermaid
graph LR
    Query[Query_Input] --> EmbedQ[Embed_Query]
    EmbedQ --> Hybrid[Hybrid_Search]
...
**Acceptance criteria**: Done when ambiguous queries (e.g., "error handling patterns") trigger sub-query generation, parallel search, and merged results; simple queries are unchanged. **Definition of Done**: Expander tests pass with mocked LLM; max sub-queries configurable (default 3).

#### Query Expansion Flow

```mermaid
graph TD
    Ambiguous[Ambiguous_Query] --> Detect[Detect_Ambiguity]
    Detect --> SubQ[Generate_SubQueries]
    SubQ --> Search1[Search_1]
    SubQ --> Search2[Search_2]
    SubQ --> Search3[Search_3]
    Search1 --> Merge[Merge_Results]
    Search2 --> Merge
    Search3 --> Merge
    Merge --> Dedup[Deduplicate_Rank]
```
...
**Parallelization**: **Parallel-safe with**: Epic 1, Epic 2, Epic 4. **Not parallel-safe with**: Epic 5 (shared: generation wrapped by tracing).

### Epic 3 Overview

Epic 3 implements the generation layer that turns retrieved context and the user query into a natural language answer with file/line citations. It covers Claude 4.5 Sonnet integration via ILLMProvider (including streaming and prompt caching), citation-focused prompt templates for explanation, dependency mapping, impact analysis, and pattern detection, and graceful fallbacks for no-results and low-confidence so the system never hallucinates when no relevant code exists.

### Dependencies

- **Epic 0**: ILLMProvider interface and config (Anthropic API key).
- **Epic 2**: Assembled context (with citations) and query object passed into the generator.

### Success Criteria

- Claude generates answers that cite file paths and line numbers in the specified format.
- Prompt caching is used for context to reduce token cost on repeated queries.
- When retrieval returns zero results or all scores are below threshold, the system returns a clear "No relevant code found" or low-confidence disclaimer instead of generating an answer.

### Risks and Mitigations

- **Hallucinated citations**: Enforce citation format in system prompt and few-shot examples; run faithfulness metrics (e.g., RAGAS) in Epic 5/6.
- **API cost**: Use prompt caching and cap context size; monitor usage in observability.

### Generation Flow

```mermaid
graph LR
    Context[Assembled_Context] --> Prompt[Prompt_Template]
    Query[User_Query] --> Prompt
    Prompt --> Claude[Claude_LLM]
    Claude --> Response[Response_Citations]
```
...
**Acceptance criteria**: Done when zero retrieval results or all scores below threshold yield a predefined fallback message or disclaimer; no LLM call is made for no-results. **Definition of Done**: Fallback tests pass; faithfulness metrics in Epic 5/6 confirm no fabricated citations.

#### No-Results and Low-Confidence Decision Flow

```mermaid
flowchart TD
    Results[Retrieval_Results] --> CheckCount{Result count}
    CheckCount -->|Zero| NoResults[Return No relevant code found]
    CheckCount -->|Greater than 0| CheckScores{Max score >= 0.3}
    CheckScores -->|No| LowConf[Add low-confidence disclaimer]
    CheckScores -->|Yes| Generate[Call LLM generate]
    NoResults --> Suggest[Suggest query refinement]
    LowConf --> Generate
    Generate --> Response[Response with citations]
```
...
**Parallelization**: **Parallel-safe with**: Epic 2, Epic 3, Epic 5. **Not parallel-safe with**: Epic 1 (shared: `src/cli/`).

### Epic 4 Overview

Epic 4 delivers the user-facing CLI that ties ingestion, retrieval, and generation together. Users run `legacylens ingest`, `legacylens query`, `legacylens show`, and `legacylens status` to index a codebase, ask questions, view file context, and check system status. The CLI uses Rich for formatted output and Pygments for COBOL syntax highlighting so that answers, snippets, and citations are readable in the terminal. This epic completes the MVP user journey.

### Dependencies

- **Epic 0**: Config and project layout; CLI framework (Typer/Click) can be added in Epic 0 or here.
- **Epic 1**: `legacylens ingest` calls the ingestion orchestrator.
- **Epic 2 and 3**: `legacylens query` calls retrieval then generation; `legacylens show` may use retrieval or direct file read.

### Success Criteria

- All four commands are invokable and documented; `query` returns LLM answer plus snippets with file/line citations and confidence indicators.
- Code snippets are syntax-highlighted (COBOL); `show <file> <line>` displays surrounding context with target line highlighted.
- CLI tests pass with mocked pipelines.

### Risks and Mitigations

- **Terminal compatibility**: Use Rich and Pygments with safe defaults so output is readable in common terminals; avoid reliance on clickable links for core flow.
- **Long responses**: Paginate or truncate very long answers if needed; keep citations always visible.

### CLI Command Surface

```mermaid
graph TD
    CLI[legacylens] --> Ingest[ingest path]
    CLI --> Query[query question]
    CLI --> Show[show file line]
    CLI --> Status[status]
    Ingest --> Orchestrator[IngestionOrchestrator]
    Query --> Retrieval[Retrieval Pipeline]
    Query --> Generation[Generation Pipeline]
    Show --> FileContext[File context ±50 lines]
    Status --> ConfigCheck[Config and index info]
```
...
**Parallelization**: **Parallel-safe with**: Epic 1, Epic 4. **Not parallel-safe with**: Epic 2, Epic 3 (shared: tracing wraps retrieval/generation).

### Epic 5 Overview

Epic 5 adds structured logging, LangSmith tracing, and RAGAS-based evaluation so developers can debug retrieval and generation, measure latency per step, and track RAG quality (context precision, recall, faithfulness). Observability is integrated across the pipeline without tight coupling, supporting the <3s latency and >70% precision targets and failure-mode analysis.

### Dependencies

- **Epics 0–4**: Pipeline exists to instrument; config may include optional LANGSMITH_API_KEY.

### Success Criteria

- Logs are structured (e.g., JSON via structlog) with trace IDs for request correlation.
- LangSmith receives traces for retrieval and generation steps with latency breakdown.
- RAGAS (or equivalent) metrics run on a synthetic test set and are documented.

### Risks and Mitigations

- **LangSmith cost/noise**: Make tracing opt-in via env; sample or filter high-volume traces if needed.
- **Metric drift**: Pin RAGAS dataset and run periodically; document in RAG Architecture Doc.
...
**Parallelization**: **No parallel** (run after Epics 1–5; validation and docs).

### Epic 6 Overview

Epic 6 validates the MVP end-to-end: E2E tests cover spec scenarios (main entry point, CUSTOMER-RECORD, CALCULATE-INTEREST, file I/O, dependencies, error handling), performance benchmarks confirm <3s query latency and <5 min ingestion for 10K LOC, documentation (README, RAG Architecture, AI Cost Analysis) is complete, deployment to Vercel is successful, and the MVP checklist is verified. This epic gates the transition to Post-MVP epics.

### Dependencies

- **Epics 0–5**: Full pipeline and CLI must be implemented and observable.

### Success Criteria

- All E2E test scenarios pass with correct citations and non-hallucinated answers; latency and ingestion benchmarks meet targets.
- README, RAG_ARCHITECTURE.md, and AI_COST_ANALYSIS.md exist and are accurate; public deployment URL is documented.
- MVP_CHECKLIST.md lists all spec requirements and all are checked off after manual QA.

### Risks and Mitigations

- **Flaky E2E**: Use stable test data and mocks where appropriate; isolate real API calls for a small smoke suite if needed.
- **Deployment env**: Document Vercel env vars and secrets; verify branch protection and deploy from main.

### MVP Validation Phases

```mermaid
graph TD
    E2E[E2E_Tests] --> Bench[Benchmarks]
    Bench --> Docs[Documentation]
    Docs --> Deploy[Deployment]
    Deploy --> Accept[Acceptance_Checklist]
    E2E --> Scenarios[Spec scenarios]
    Bench --> Latency[Latency Ingestion Precision]
    Docs --> Readme[RAG_Arch Cost_Analysis]
    Accept --> QA[Manual QA]
```
...
The following epics extend beyond MVP to implement advanced features for early/final submission. They can be scheduled in parallel after Epic 6 is complete; Epic 7 builds on the MVP core for code understanding, while Epics 8–10 focus on optimization, automation, and outreach.

### Post-MVP Epic Relationship

```mermaid
graph TD
    MVP[Epic_0_to_6_MVP_Core] --> E7[Epic_7_Advanced_Features]
    MVP --> E8[Epic_8_Performance]
    MVP --> E9[Epic_9_CI/CD]
    MVP --> E10[Epic_10_Demo]
    E7 --> Deps[Dependency_Impact_Pattern_Logic]
    E8 --> Cache[Cache_Index_Opt]
    E9 --> IngestWF[Auto_Ingest_Deploy]
    E10 --> Video[Demo_Social]
```
...
**Parallelization**: **Parallel-safe with**: Epic 8, Epic 9, Epic 10 (Post-MVP; disjoint: graph/analysis/cli vs cache/index vs .github).

### Epic 7 Overview

Epic 7 adds four advanced code-understanding capabilities: dependency mapping via Neo4j GraphRAG (call graphs, paragraph/file relationships), impact analysis (what breaks if a function changes), pattern detection (e.g., error handling patterns via semantic search), and business logic extraction (LLM-summarized rules with citations). These features differentiate LegacyLens for early/final submission and rely on the MVP retrieval and generation pipeline.

### Dependencies

- **Epics 0–6**: MVP pipeline and CLI; Neo4j is introduced here (config from Epic 0 optional).
- **Epic 1**: Parser metadata (CALL statements, scope) feeds the graph and impact analyzer.

### Success Criteria

- `legacylens deps <name>` returns a dependency tree; `legacylens impact <name>` lists direct/transitive callers and data deps.
- `legacylens patterns "<description>"` returns matching code blocks; `legacylens extract-logic <file>` returns business rules with citations.
- All four features have unit tests and are documented.

### Risks and Mitigations

- **Neo4j schema creep**: Define a minimal schema (Functions/Paragraphs/Files, CALLS/CONTAINS/MODIFIES) and extend only when needed.
- **LLM cost for logic extraction**: Limit context per file; use prompt caching.

### Advanced Features Map

```mermaid
graph LR
    Deps[Dependency_Mapping] --> Neo4j[Neo4j_GraphRAG]
    Impact[Impact_Analysis] --> Neo4j
    Pattern[Pattern_Detection] --> Search[Semantic_Search]
    Logic[Business_Logic] --> LLM[LLM_Extract]
```
...
**Parallelization**: **Parallel-safe with**: Epic 7, Epic 9, Epic 10 (Post-MVP; disjoint from graph/analysis and .github).

### Epic 8 Overview

Epic 8 optimizes the MVP for production load: multi-level caching (full query result cache with hash key, LRU eviction), Pinecone metadata index tuning and batch-size optimization, and any query preprocessing that reduces latency or cost. These changes support the 100K-user cost and latency projections.

### Dependencies

- **Epics 0–6**: Pipeline and Redis in place; Pinecone index in use.

### Success Criteria

- Query result cache reduces redundant embedding + search + generation for repeated queries; LRU eviction keeps cache within size limit.
- Pinecone metadata filters and batch sizes are tuned and benchmarked; query speed improvements documented.

### Risks and Mitigations

- **Stale cache**: Use TTL (e.g., 24h) and consider invalidation on index update (Epic 9).
- **Over-tuning**: Measure before/after; avoid premature optimization.
...
**Parallelization**: **Parallel-safe with**: Epic 7, Epic 8, Epic 10 (Post-MVP; only adds `.github/workflows/ingest.yml`).

### Epic 9 Overview

Epic 9 automates ingestion on code changes (e.g., GitHub Actions triggered on push to main for specific paths) and streamlines deployment so the index and backend stay current with the repo. It reduces manual re-ingestion and supports continuous delivery.

### Dependencies

- **Epics 0–6**: Ingestion CLI and deployment (Vercel) exist; GitHub repo and secrets configured.

### Success Criteria

- Push to main (or selected paths) triggers ingestion workflow; Pinecone index is updated.
- Deployment is repeatable and documented; no manual steps required for standard updates.

### Risks and Mitigations

- **Secret availability in CI**: Use GitHub Secrets for API keys; restrict workflow to necessary repos/branches.
- **Long-running ingest in CI**: Set timeout and consider async notification if ingestion exceeds job limit.
...
**Parallelization**: **Parallel-safe with**: All (no code changes; demo/social only).

### Epic 10 Overview

Epic 10 covers non-technical deliverables: a 3–5 minute demo video (script, record, edit, upload) and a social post (description, screenshots, @GauntletAI, demo link) to meet submission and outreach requirements. No code changes; manual tasks with clear deliverables.

### Dependencies

- **Epic 6**: MVP deployed and stable so demo and screenshots are representative.

### Success Criteria

- Demo video URL is available and linked from README; video shows ingestion, queries, and results.
- Social post is published with required tags and links; URL documented.

### Risks and Mitigations

- **Scope creep**: Keep video to 3–5 minutes; focus on one codebase and a few representative queries.
...
#### A.3: Chunk Lifecycle (Ingestion to Retrieval)

```mermaid
sequenceDiagram
    participant File as Raw_File
    participant Parser
    participant Chunker
    participant Embedder
    participant Cache as Redis_Cache
    participant Indexer as Pinecone

    File->>Parser: parse_file
    Parser->>Chunker: parsed_units
    Chunker->>Chunker: chunk_id metadata
    Chunker->>Embedder: chunks
    Embedder->>Cache: content_hash lookup
    alt cache hit
        Cache-->>Embedder: embedding
    else cache miss
        Embedder->>Embedder: Voyage API
        Embedder->>Cache: store embedding
    end
    Embedder->>Indexer: upsert vectors + metadata
    Indexer-->>Indexer: indexed
```
